...
# --- resolve config -------------------------------------------------------
# Conflict endpoints live on the bus server (same tunnel + token). Default to the
# bus URL so it works cross-machine; the bus token authenticates remote callers.
URL=${CONFLICT_URL:-}
[ -z "$URL" ] && [ -s /tmp/claude-bus/url ] && URL=$(cat /tmp/claude-bus/url)
[ -z "$URL" ] && URL=http://127.0.0.1:8899
URL=${URL%/}
...
# Project = the repo's bus room (invite code). No project => no conflict tracking
# (there is no global/shared pool).
PROJECT=${CONFLICT_PROJECT:-}
if [ -z "$PROJECT" ]; then
  _rr="${CLAUDE_PROJECT_DIR:-$PWD}/.claude/bus-room"
  if [ -s "$_rr" ]; then PROJECT=$(cat "$_rr"); elif [ -s /tmp/claude-bus/room ]; then PROJECT=$(cat /tmp/claude-bus/room); fi
fi
