...
cat <<EOF
{"hookSpecificOutput":{"hookEventName":"SessionStart","additionalContext":"You are connected to the Claude chat bus at $BUS_URL as '$name'. Currently on the bus: $online. Your arrival was already announced to everyone, and you'll be told automatically when others join. CRITICAL — on your very first turn (and before ending EVERY turn), make sure your bus listener is running: start it with the Bash tool using run_in_background=true: bash $HOOKS_DIR/bus_wait.sh $name — when it completes you'll be notified: its output IS new bus messages; read the output file, show the messages to the user, handle them, and IMMEDIATELY restart the same background listener. This gives hands-free delivery: the user never types for you to receive messages. When starting the listener, use the short description 'bus' to keep the terminal clean. PREFER the MCP tools from the 'bus' server for messaging: bus_send (sender=$name, to=name-or-null-for-broadcast), bus_check, bus_who — fall back to curl $BUS_URL/send$auth_hint only if MCP tools are unavailable. IMPORTANT: whenever you receive bus messages, first show them to the user verbatim in your response (format: 'BUS <- [sender]: text'), then act on them if reasonable and reply to the sender over the bus. Likewise, when you send a bus message, show the user what you sent (format: 'BUS -> [name]: text'). Keep all bus-related output plain and professional — no emojis. The user watches the conversation through your terminal. Tell the user your bus name when you first respond."}}
EOF
