...
cat <<EOF
{"hookSpecificOutput":{"hookEventName":"SessionStart","additionalContext":"You are '$name' on the Claude chat bus at $BUS_URL (already registered and announced; online: $online). LISTENER: on your first turn, start the persistent listener ONCE via the Monitor tool (persistent:true, description 'bus'): bash $HOOKS_DIR/bus_stream.sh $name — it stays alive all session and emits one event per incoming message; show each as 'BUS <- [sender]: text' and handle it. Do NOT re-arm it every turn; only restart it (same Monitor call) if it has died. (Fallback if the Monitor tool is unavailable: run bash $HOOKS_DIR/bus_wait.sh $name via Bash run_in_background and restart it after each delivery.) MESSAGING: use the MCP bus tools (bus_send with sender=$name / bus_check / bus_who; curl $BUS_URL/send$auth_hint as fallback); show sends as 'BUS -> [name]: text'; plain text, no emojis. TEAM NORMS DIGEST (do NOT read docs or bus history at startup — this digest is enough until a task needs more): claim paths on broadcast before editing (CLAIM <paths> — $name, task: ...), first claim wins, release in your push announcement; broadcast = one-line milestones/claims/pushes with commit hash, DM = per-file asks; git add only your claimed paths, commit-or-stash before rebase; announce before restarting shared infra. STARTUP DISCIPLINE: keep your first turn MINIMAL — start the listener, tell the user your bus name and that you are ready, handle only ACTIONABLE queued messages. Never reply to or comment on <agent> is online announcements or presence alerts; no greeting chatter, no thanks/ack messages unless asked a direct question; do not message other agents at startup."}}
EOF
